...
def markdown_table(df: pd.DataFrame) -> str:
    ...
    for row in df.itertuples(index=False):
        rows.append(
            f"| {row.model} | {row.horizon} | {row.mse_mean:.4f} | {row.mse_std:.4f} | {row.mae_mean:.4f} | {row.mae_std:.4f} |"
        )
    return "\n".join(rows)
...
def main() -> None:
    ...
    if summary_path.exists():
        ...
        table = "_Run `python src/train.py --horizons 90 365 --seeds 0 1 2 3 4 --epochs 80` to generate results._"
...
本节从 LSTM、Transformer 和本文提出的 CNN-Transformer 三种方法进行横向比较。上述图像均为预测电量 power 与真实值 Ground Truth 的曲线截图，其中蓝色曲线表示真实值，橙色曲线表示模型预测值。
...
本文提出的 CNN-Transformer 结构具有一定新颖性，即先通过卷积抽取局部用电模式，再用 Transformer 建模长期依赖；即使短期预测性能不总是最优，其原因主要在于卷积平滑会削弱尖峰拟合、样本规模有限以及天气变量粒度较粗。

工具使用说明：本报告撰写过程中使用了 ChatGPT/Codex 辅助整理文字和代码结构；模型设计、实验运行与结果分析仍需以仓库中的可复现实验输出为准。完整参考文献列于下方，避免未标注引用。
